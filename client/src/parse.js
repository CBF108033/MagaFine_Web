...
export const postTextLimit = (text, inputText) => {
    ...
    if (Array.isArray(text)) { //如果是陣列，則將陣列內的字串相加，並且將圖片節點移除，否則直接將字串指定給shortList
        text.map(i => {
            if (i.props?.children || null) {
                if (i.props.children.type !== 'img') {
                    shortList += i.props.children
                }
            }
        })
    } else if (typeof text === 'object' && text !== null){
        shortList += text.props.children
    } 
    else {
        shortList = []
        return shortList
