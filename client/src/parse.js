...
export const postTextLimit = (text, inputText) => {
    ...
    if (inputText === "") {//沒有搜尋字串時，字數大於60則顯示前75字，小於60則顯示全部
        res = shortList.length > 60 ? shortList.substring(0, 75) + `...` : shortList
    } else {//有搜尋字串時，字數大於60則顯示搜尋字串前後25字，小於60則顯示全部
    
        if (shortList > 60) {//有搜尋字串時，字數大於60則顯示搜尋字串前後25字，小於60則顯示全部
            res = (shortList.substring(0, 75) + `...`).replace(inputText, `<span class="highlight">${inputText}</span>`)
